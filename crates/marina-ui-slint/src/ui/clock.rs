...
pub(crate) fn initialize(window: &MainWindow) {
    let initial_twelve_hour = twelve_hour();
    window.global::<ClockState>().set_time(SharedString::from(
        marina_ui_slint::clock_format::current_time_string(initial_twelve_hour),
    ));
    ...
    slint::Timer::single_shot(std::time::Duration::ZERO, move || {
        tokio::spawn(async move {
            ...
            let mut last = marina_ui_slint::clock_format::current_time_string(twelve_hour());
            apply_time(&weak, last.clone());
            let mut interval = tokio::time::interval(TICK_INTERVAL);
            loop {
                interval.tick().await;
                let now = marina_ui_slint::clock_format::current_time_string(twelve_hour());
                if now != last {
                    last = now.clone();
                    apply_time(&weak, now);
                }
