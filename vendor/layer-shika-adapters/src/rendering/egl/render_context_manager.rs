...
impl RenderContextManager {
    pub fn new(display_id: &ObjectId) -> Result<Rc<Self>> {
        ...
        // Embedded Wayland drivers commonly expose EGL without desktop OpenGL.
        // Pin both config selection and context creation to GLES 2 instead of
        // relying on the first config's advertised API set.
    ...
    fn try_create_surfaceless_context(
        ...
        let context_attributes = gles_context_attributes();
        let not_current =
...
fn gles_context_attributes() -> ContextAttributesBuilder {
    ContextAttributesBuilder::new().with_context_api(ContextApi::Gles(Some(Version::new(2, 0))))
}
