...
pub struct SurfaceRenderer<W: RenderableWindow> {
    ...
    viewport_destination: Option<PhysicalSize>,
}
...
impl<W: RenderableWindow> SurfaceRenderer<W> {
        ...
        match mode {
            ScalingMode::FractionalWithViewport => {
                ...
                if let Some(viewport) = &self.viewport {
                    let destination = self.viewport_destination.unwrap_or_else(|| {
                        PhysicalSize::new(dimensions.logical_width(), dimensions.logical_height())
                    });
                    viewport.set_destination(destination.width as i32, destination.height as i32);
                }
            }
    ...
    pub fn update_size(&mut self, width: u32, height: u32, scale_factor: f32) {
        self.update_size_with_viewport_destination(width, height, scale_factor, None);
    }

    pub fn update_size_with_viewport_destination(
        &mut self,
        width: u32,
        height: u32,
        scale_factor: f32,
        ...
        if let Some(destination) = viewport_destination {
            self.viewport_destination = Some(destination);
        }
