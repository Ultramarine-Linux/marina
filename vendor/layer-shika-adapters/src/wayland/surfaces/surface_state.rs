...
pub struct SurfaceState {
    ...
    #[allow(dead_code)]
    pointer: Option<ManagedWlPointer>,
}
...
impl SurfaceState {
    pub fn new(builder: SurfaceStateBuilder) -> Result<Self> {
        ...
        let pointer = builder
            .pointer
            .map(|pointer| ManagedWlPointer::new(pointer, Rc::clone(&connection)));
    ...
    pub fn update_size_from_compositor(
        &mut self,
        logical_width: u32,
        logical_height: u32,
        ...
        let scale_factor = self.event_context.borrow().scale_factor();
        self.rendering.update_size_from_compositor(
            logical_width,
            logical_height,
            scale_factor,
            PhysicalSize::new(compositor_width, compositor_height),
        );
    }
