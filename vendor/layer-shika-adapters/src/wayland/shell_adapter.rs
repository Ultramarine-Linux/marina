...
impl WaylandShellSystem {
    ...
    fn create_output_setups(
        ...
        for (index, output) in global_ctx.outputs.iter().enumerate() {
            ...
            if let Some(pointer) = pointer {
                builder = builder.with_pointer(Rc::clone(pointer));
            }
    ...
    fn init_state(
        ...
        // A wl_pointer may only be requested after the seat advertises pointer
        // capability. Controller-only handheld seats can omit it entirely.
        let pointer: Option<Rc<WlPointer>> = None;
    ...
    fn create_output_manager(params: &OutputManagerParams<'_>) -> OutputManager {
        let manager_context = OutputManagerContext {
            ...
            pointer: params.pointer.map(Rc::clone),
